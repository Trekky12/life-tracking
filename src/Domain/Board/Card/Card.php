...
class Card extends \App\Domain\DataObject {
    ...
    public function parseData(array $data) {
        ...
        $this->time = $this->exists('time', $data) ? Utility::filter_string_polyfill($data['time']) : null;
         ...
         * Clean date/time
         */
        if (!empty($this->date) && !is_null($this->date) && !preg_match("/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/", $this->date)) {
            $this->date = date('Y-m-d');
        }

        if (!empty($this->time) && !is_null($this->time) && !preg_match("/^[0-9]{2}:[0-9]{2}(:[0-9]{2})?$/", $this->time)) {
