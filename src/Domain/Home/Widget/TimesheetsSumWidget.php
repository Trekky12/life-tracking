...
class TimesheetsSumWidget implements Widget {
    ...
    public function getContent(?WidgetObject $widget = null) {
        ...
        $totalSeconds = $this->sheet_mapper->tableSum($id, $range["min"], $range["max"], $categories, $invoiced, $billed, $payed, $happened, $customer);

        $sum = DateUtility::splitDateInterval($totalSeconds);
        if ($project->has_duration_modifications > 0 && $totalSeconds > 0) {
            $totalSecondsModified = $this->sheet_mapper->tableSum($id, $range["min"], $range["max"], $categories, $invoiced, $billed, $payed, $happened, $customer, "%", "t.duration_modified");
