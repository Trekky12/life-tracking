...
class AdminTest extends BaseTestCase {
    ...
    protected function getElementInTable($body) {
        $matches = [];
        $re = '/<tr>\s*<td>\s*x\s*<\/td>\s*<td>([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2})<\/td>\s*<td>([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2})<\/td>\s*<td>(.?)*<\/td>\s*<td>(.?)*<\/td>\s*<td>' . $this->LOCAL_IP . '<\/td>\s*<td>\s*<a href="#" data-url="' . str_replace('/', "\/", $this->uri_delete) . '(?<id_delete>[a-z\/0-9]*)" class="btn-delete">.*?<\/a>\s*<\/td>\s*<\/tr>/';
