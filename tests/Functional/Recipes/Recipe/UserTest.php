...
class UserTest extends BaseTestCase {
    ...
    protected function getIDFromView($body) {
        $matches = [];
        $re = '/<a href="' . str_replace('/', "\/", $this->uri_edit) . '(?<id_edit>[0-9]*)"><button class="button">.*?<\/button><\/a>\s*<a href="#" data-url="' . str_replace('/', "\/", $this->uri_delete) . '(?<id_delete>[0-9]*)" class="btn-delete"><button class="button">.*?<\/button><\/a>/';
        preg_match($re, $body, $matches);
