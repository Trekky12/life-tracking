...
class RecipesMealplansTestBase extends BaseTestCase {
    ...
    protected function getParent($body, $name) {
        $matches = [];
        $re = '/<tr>\s*<td>\s*<a href="\/recipes\/mealplans\/(?<hash>.*)\/view\/">' . preg_quote($name) . '<\/a>\s*<\/td>\s*<td>\s*<a href="' . str_replace('/', "\/", $this->uri_edit) . '(?<id_edit>[0-9]*)">.*?<\/a>\s*<\/td>\s*<td>\s*<a href="#" data-url="' . str_replace('/', "\/", $this->uri_delete) . '(?<id_delete>[0-9]*)" class="btn-delete">.*?<\/a>\s*<\/td>\s*<\/tr>/';
    ...
    protected function getParents($body) {
        $matches = [];
        $re = '/<tr>\s*<td>\s*<a href="\/recipes\/mealplans\/(?<hash>.*)\/view\/">(?<name>.*)<\/a>\s*<\/td>\s*(<td>[\s]*<\/td>\s*)*<\/tr>/';
