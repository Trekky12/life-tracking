...
class OwnerTest extends TimesheetTestBase {
    ...
    public function testList() {
        ...
        $this->assertStringContainsString('<table id="project_customers_table"', $body);
    }
    ...
    public function testGetAddElement() {
        ...
        $body = (string) $response->getBody();
        $this->assertStringContainsString('<form class="form-horizontal" id="projectCustomersForm" action="' . $this->getURIChildSave($this->TEST_PROJECT_HASH) . '" method="POST">', $body);
    }
    ...
    public function testAddedElement($data) {
        ...
        $body = (string) $response->getBody();
        $row = $this->getCustomerElementInTable($body, $data, $this->TEST_PROJECT_HASH);
    ...
    public function testGetElementCreatedEdit(int $entry_id, array $data) {
        ...
        $matches = [];
        $re = '/<form class="form-horizontal" id=\"projectCustomersForm\" action="(?<save>[\/a-zA-Z0-9]*)" method="POST">.*<input name="id" id="entry_id" type="hidden" value="(?<id>[0-9]*)">/s';
    ...
    public function testGetElementUpdated(array $result_data) {
        ...
        $row = $this->getCustomerElementInTable($body, $result_data, $this->TEST_PROJECT_HASH);
