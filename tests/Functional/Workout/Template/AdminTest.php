...
class AdminTest extends BaseTestCase {
    ...
    protected function getElementInTable($body, $data) {
        $matches = [];
        $re = '/<tr>\s*<td><a href="\/workouts\/templates\/(?<hash>.*)\/view\/">' . preg_quote($data["name"]) . '<\/a><\/td>\s*<td>[0-9]*<\/td>\s*<td>[0-9]*<\/td>\s*<td><a href="' . str_replace('/', "\/", $this->uri_edit) . '(?<id_edit>[0-9]*)">.*?<\/a><\/td>\s*<td><a href="#" data-url="' . str_replace('/', "\/", $this->uri_delete) . '(?<id_delete>[0-9]*)" class="btn-delete">.*?<\/a>\s*<\/td>\s*<\/tr>/';
        preg_match($re, $body, $matches);
